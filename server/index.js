...
app.post('/api/deliveries', async (req, res) => {
  try {
    ...
    if (!pickup_address || !dropoff_address || !dropoff_phone_number) {
      return res.status(400).json({
        error: 'pickup_address, dropoff_address, and dropoff_phone_number are required',
      });
    }
  ...
  try {
    ...
    if (deliveryId) {
      ...
      if (deliveries.has(deliveryId)) {
        ...
        delivery.delivery_status = event.delivery_status || delivery.delivery_status;
...
app.get('/api/deliveries/:id/stream', (req, res) => {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      if (['delivered', 'cancelled', 'returned'].includes(delivery.delivery_status)) {
...
app.listen(PORT, () => {
  ...
  try {
    ...
    console.warn('Set your credentials in server/.env to enable DoorDash integration');
  }
});
